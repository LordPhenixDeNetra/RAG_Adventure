...
async def ask_question(request: QuestionRequest):
    ...
    try:
        ...
        # Génération de la réponse (non-streaming)
...
@app.post("/ask-question-stream",
          ...
          description="Pose une question avec réponse en temps réel (streaming)")
    ...
    try:
        ...
        if not results["documents"] or not results["documents"][0]:
            async def no_context_stream():
                ...
                yield f"data: {json.dumps({'content': message, 'done': True})}\n\n"
        ...
        # Préparation du contexte
        context = "\n".join(results["documents"][0])
        prompt_text = prompt_template.format(context=context, question=request.question)
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=300.0) as client:
                    async with client.stream(
                        ...
                        if response.status_code != 200:
                            yield f"data: {json.dumps({'error': 'Erreur de connexion avec Ollama'})}\n\n"
                            return
                ...
                yield f"data: {json.dumps({'error': f'Erreur: {str(e)}'})}\n\n"
